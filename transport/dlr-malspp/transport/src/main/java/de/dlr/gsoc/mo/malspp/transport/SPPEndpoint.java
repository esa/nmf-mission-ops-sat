...
//        private final Thread outgoingThread;
	...
	public SPPEndpoint(
			...
			final SPPSocket sppSocket) throws MALException {
		this.protocol = protocol;
		this.transport = transport;
		// PENDING: Testbed throws NullpointerExceptions, if an endpoint, that is created with
		// null as local name actually returns null when asked for the name. Here: Set the hash code
		// as local name.
...
//			throw new MALException(ENDPOINT_CLOSED);
	...
	/**
	 * Creates an error message in reply to another MAL message. If returning an error message is
	 * not allowed by the interaction type and stage of the original MAL message, then null is
	 * returned.
	 *
	 ...
	 * @return A MALMessage representing the error, with the isErrorMessage flag set, the timestamp
	 * of the error generation and the correct interaction stage for returning an error. Null, if it
	...
	@Override
	public void sendMessage(final MALMessage msg) throws IllegalArgumentException, MALTransmitErrorException, MALException {
		if (isClosed) {
			throw new MALException(ENDPOINT_CLOSED);
		}
		if (msg == null) {
...
//                                this.outgoingQueue.put(spacePackets);
	...
	@Override
	public void sendMessages(final MALMessage[] msgList) throws
			IllegalArgumentException, MALTransmitMultipleErrorException, MALException {
		if (isClosed) {
			throw new MALException(ENDPOINT_CLOSED);
		}
		if (msgList == null) {
	...
	public void setMessageListener(final MALMessageListener listener) throws MALException {
		...
		// PENDING: 5.2.4.7.4 MAL Java API only says 'listener' shall not be NULL, but doesn't say
