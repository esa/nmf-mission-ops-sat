...
public class SPPMessageHeader implements MALMessageHeader {
    ...
    private int initMessageHeader(final ByteArrayInputStream is, final boolean isTCpacket, final int sequenceFlag,
        ...
        Short sourceIdentifier = bs.get(7) ? ((UOctet) eis.readElement(new UOctet(), null)).getValue() : null;

        Short destinationIdentifier = bs.get(6) ? ((UOctet) eis.readElement(new UOctet(), null)).getValue() : null;
        ...
        UInteger priority = bs.get(5) ? (UInteger) eis.readElement(new UInteger(), null) : config.priority();
        setPriority(priority);

        Time timestamp = bs.get(4) ? (Time) eis.readElement(new Time(), null) : Configuration.DEFAULT_TIMESTAMP;
        setTimestamp(timestamp);

        Identifier networkZone = bs.get(3) ? (Identifier) eis.readElement(new Identifier(), null)
                : config.networkZone();
        setNetworkZone(networkZone);

        Identifier sessionName = bs.get(2) ? (Identifier) eis.readElement(new Identifier(), null)
                : config.sessionName();
        setSessionName(sessionName);

        IdentifierList domain = bs.get(1) ? (IdentifierList) eis.readElement(new IdentifierList(), null)
                : config.domain();
        setDomain(domain);

        Blob authenticationId = bs.get(0) ? (Blob) eis.readElement(new Blob(), null) : config.authenticationId();
