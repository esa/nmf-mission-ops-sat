...
public class SPPMessageHeader implements MALMessageHeader {
	...
	public SPPMessageHeader(
			...
			final Boolean isErrorMessage) throws IllegalArgumentException {
		if (uriFrom == null
				...
				// PENDING: Testbed expects no exception if interactionStage == null, although this
				// is demanded by 5.2.4.4.4 MAL Java API. Here: If interactionStage == null set it
				// to UOcctet(0).
	...
	private int initMessageHeader(
		...
		// Read segment counter only if it was encoded, but ignore its value. SPPSegmenter will
		// read this field again and uses it for packet ordering. This could be solved in a more
	...
	private static final UOctet[] SDU_STAGES = new UOctet[]{
		new UOctet((short) 0), // interaction stage for SEND not specified, set to 0
