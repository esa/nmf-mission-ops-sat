...
public class CCSDSTime {
    ...
    static {
        try {
            org.orekit.data.DataProvidersManager.getInstance().addProvider(new org.orekit.data.ClasspathCrawler(
                OREKIT_UTC_TAI_FILE));
     ...
     * @throws MALException
     */
    public CCSDSTime(final byte[] pField, final String epoch, final String timeScale, final int unitMultiplier)
        throws MALException {
     ...
     * @throws MALException
     */
    public CCSDSTime(final String pField, final String epoch, final String timeScale, final int unitMultiplier)
        throws MALException {
        ...
        try {
            switch (timeCode) {
                case CUC:
                    ...
                    final AbsoluteDate possiblyWrongTime = AbsoluteDate.parseCCSDSUnsegmentedTimeCode(pField[0],
                        pField.length == 2 ? pField[1] : 0, tField, epoch);
                    if (1 == unitMultiplier) {
     ...
     * @throws MALException
     */
    public byte[] getEncodedTime(final AbsoluteDate coarseTime, final AbsoluteDate fineTime,
        final boolean mayBeNegative) throws MALException {
    ...
    private byte[] encodeCUC(final AbsoluteDate coarseTime, final AbsoluteDate fineTime) throws MALException {
        ...
        final long fineSeconds = Math.round(fineTime.durationFrom(coarseTime) * unitMultiplier * (1L << (8 *
            nFractionalOctets)));
