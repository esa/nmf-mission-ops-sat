...
public class Configuration {
	...
	// 'Undocumented' configuration parameters to limit the range of used instance identifiers
	...
	// Mapping configuration paramter property names (per APID/APID qualifier) for encoding.
	// Only used to parse XML configuration file in order to forward those properties to the
	// encoding layer. The encoding layer has to implement its own interpretation of those properties.
	...
	/**
	 * Return a map containing all properties that are effective for a specific application
	 * identified by APID qualifier and APID. Please note that the application is not necessarily
	 * identified by the sending or receiving endpoint but is dependent on the packet type (TM/TC).
	 *
...
//                    "The apid:" + primaryApid + " is not defined in the mcp file. Taking the values for apid=-1"); 
	...
	/**
	 * Returns a newly created Map containing the union of two property maps. If a value is present
	 * in both maps the value of newProperties will be taken. Any map may be null.
	 *
